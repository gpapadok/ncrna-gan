...
def _one_hot_to_base(oh):
    ### save list of sequences to fasta
...
def _save_to_fasta(sequences, labels, name):
    with open(name, 'w') as fasta:
        for j, seq in enumerate(sequences):
            fasta.write(f">{j} artificial hsa ncrna sequence | {str(labels[j])}\n")
            fasta.write(seq + "\n")

            
def _decode_sequence(one_hot_seq):
    ### functions to decode sequences from one_hot to string
...
if __name__=='__main__':
    ...
    if torch.cuda.is_available():
        gen.cuda()
    gen.load_state_dict(torch.load(gen_wfile, map_location=torch.device('cuda:0') \
